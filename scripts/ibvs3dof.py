...
class ImageBasedVisualServoing(object):
    ...
    def __init__(self):
        ...
        if self.six_dof:
            ...
            # lambda from Corke eq. 15.11 (turning param)
            self.lam = np.array([0.5, 0.5, 0.5], dtype=np.float32).reshape(3,1)  # 4x1
    ...
    def level_frame_corners_callback(self, msg):
        ...
        if self.six_dof:
            ...
            u1 = msg.data[8]
            v1 = msg.data[9]
            Jp1 = np.array([[-self.f/self.z_c, 0.0, v1],
                            [0.0, -self.f/self.z_c, -u1]])  # 2x3
        
            u2 = msg.data[10]
            v2 = msg.data[11]
            Jp2 = np.array([[-self.f/self.z_c, 0.0, v2],
                            [0.0, -self.f/self.z_c, -u2]])  # 2x3

            u3 = msg.data[12]
            v3 = msg.data[13]
            Jp3 = np.array([[-self.f/self.z_c, 0.0, v3],
                            [0.0, -self.f/self.z_c, -u3]])  # 2x3

            u4 = msg.data[14]
            v4 = msg.data[15]
            Jp4 = np.array([[-self.f/self.z_c, 0.0, v4],
                            [0.0, -self.f/self.z_c, -u4]])  # 2x3
        ...
        if self.six_dof:
            ...
            self.vel_cmd_msg.angular.z = self.saturate(rdot_des[2][0], self.psidot_max, -self.psidot_max)
